...
def cal_curve(queue1, queue2):
    ...
    while True:
        ...
        if isMinEd and len(buffer)>180:
            ...
            print("角度为", math.atan(offset_main / offset_vice)*180 / np.pi)
            # plt.show()
            print("生产角度数据")
            queue1.put(round(math.atan(offset_main / offset_vice)*180 / np.pi,2))
            print("生产绳长数据")
            queue2.put(round(cal_l(period_time),2))
            # 清空buffer下次计算
            voice(round(cal_l(period_time),2),round(math.atan(offset_main / offset_vice)*180 / np.pi,2))
            buffer_main.clear()
            buffer_vice.clear()
            break
